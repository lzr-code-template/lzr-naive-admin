...
const submitForm = async(formEl: FormInstance | undefined) => {
  ...
  await formEl.validate((valid, fields) => {
    if (valid) {
      ...
      api.get('/system/account/updateAdmin', {
        id: route.params.id,
        mobile: form.mobile,
        name: form.name,
        role: form.role.join(',')
...
// 获取权限列表
try {
  const [roleRes, detailRes] = await Promise.all([
    api.get('/system/account/getRoleList'),
    api.get('/system/account/updateAdminUserDetail', { id: route.params.id })
  ])
  if (roleRes.code === 200) {
  ...
  if (detailRes.code === 200) {
    ...
    form.mobile = detailRes.data.mobile
    form.role = JSON.parse(detailRes.data.role).map((str: string) => parseInt(str, 10))
  }
} catch (error) {
